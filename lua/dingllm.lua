...
function M.make_ollama_spec_curl_args(opts, prompt, system_prompt)
  local url = opts.ollama_url or 'http://localhost:11434/api/generate'
  local data = {
    model = opts.model or 'llama3.2:3b',
    prompt = prompt,
    system = system_prompt,
    stream = true,
  }
...
function M.handle_ollama_spec_data(data_stream)
  ...
  elseif success and json.error then
    print('Ollama error:', json.error)
  else
    print('Failed to decode JSON or unexpected response format:', data_stream)
  end
end
  ...
  active_job = Job:new {
    ...
    on_exit = function(j, return_val)
      if return_val ~= 0 then
        print('Curl job failed with return value:', return_val)
      end
      active_job = nil
    end,
  }
...
function M.invoke_ollama_and_stream_into_editor(opts, make_curl_args_fn, handle_data_fn)
  vim.api.nvim_clear_autocmds { group = group }
  local prompt = M.get_prompt(opts)
  local system_prompt = opts.system_prompt or 'You are a helpful assistant.'
  local args = make_curl_args_fn(opts, prompt, system_prompt)
  ...
  local debug_log = io.open(vim.fn.expand '~/ollama_debug.log', 'w')

  active_job = Job:new {
    command = 'curl',
    args = args,
    on_stdout = function(_, out)
      debug_log:write('Stdout: ' .. out .. '\n')
      debug_log:flush()
      if out and out ~= '' then
        handle_data_fn(out)
      end
    ...
    on_stderr = function(_, err)
      debug_log:write('Stderr: ' .. err .. '\n')
      debug_log:flush()
      print('Curl error:', err)
    end,
    on_exit = function(j, return_val)
      debug_log:write('Job exited with return value: ' .. return_val .. '\n')
      debug_log:close()
      if return_val ~= 0 then
        print('Curl job failed with return value:', return_val)
      end
